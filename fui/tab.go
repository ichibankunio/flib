...
func NewTab(n int, selected int, centerX int, y int, width int, height int, fontface font.Face, txtClr, clrBound, clrBg color.Color) *Tab {
	...
	for i := 0; i < n; i ++ {
		...
		b := NewButton("", vec2.New(float64(centerX) - float64(width*n)/2 + float64(width) / 2  + float64(i * width), float64(y)), vec2.New(float64(width), float64(height)), fontface, theme, txtClr, clrBound, clrBg)
		if i == selected {
